...
export const SettingsPage: React.FC = () => {
  ...
  useEffect(() => {
    api.getSettings().then(setBackendSettings).catch(() => setBackendSettings(null));
  }, []);
    ...
    <div className="space-y-8 font-sans max-w-5xl mx-auto">
      <div className="flex items-center justify-between pb-2 border-b border-sky-200">
        <div>
          ...
          <p className="text-xs text-sky-800 font-medium mt-0.5">
            Workspace preferences and backend configuration.
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 border-b border-sky-200 pb-2 text-xs font-bold overflow-x-auto">
        {[
          ...
          { id: 'models', label: 'Models & Config', icon: Cpu },
        ...
        <GlassCard gradient className="p-6 space-y-4 border-sky-300">
          ...
          <div className="p-4 rounded-2xl bg-white/70 border border-sky-200 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-sky-900 font-medium mt-0.5">CYBERNEX sovereign glass theme active.</p>
            </div>
        ...
        <GlassCard className="p-6 space-y-4 border-sky-300">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-bold">
            <div>
              ...
              <input type="text" readOnly value="operator@cybernex.local" className="w-full p-3 rounded-xl glass-input text-sky-950" />
            </div>
            <div>
              <label className="text-sky-900 block mb-1">Role</label>
              <input type="text" readOnly value="Operator" className="w-full p-3 rounded-xl glass-input text-emerald-700" />
            </div>
          </div>
        </GlassCard>
      )}
      ...
      {activeTab === 'models' && (
        <GlassCard className="p-6 space-y-3 border-sky-300">
          <h3 className="text-xs font-bold text-sky-900 uppercase">Configured Models & Endpoints</h3>
          {backendSettings ? (
            <div className="space-y-2 text-xs font-medium text-sky-950">
              <div><span className="font-bold">Ollama URL:</span> {backendSettings.OLLAMA_URL}</div>
              <div><span className="font-bold">Qdrant URL:</span> {backendSettings.QDRANT_URL}</div>
              <div><span className="font-bold">General Model:</span> {backendSettings.GENERAL_MODEL}</div>
              <div><span className="font-bold">Coding Model:</span> {backendSettings.CODING_MODEL}</div>
              <div><span className="font-bold">Vision Model:</span> {backendSettings.VISION_MODEL}</div>
              <div><span className="font-bold">Embedding Model:</span> {backendSettings.EMBEDDING_MODEL}</div>
            </div>
          ) : (
            <p className="text-xs text-sky-800 font-bold">Loading backend configuration...</p>
          )}
        </GlassCard>
      )}
      ...
      {activeTab === 'security' && (
        <GlassCard className="p-6 space-y-2 border-emerald-300">
          ...
          <p className="text-xs text-emerald-700 font-bold">All model inference and vector RAG operations run locally.</p>
        </GlassCard>
      )}
